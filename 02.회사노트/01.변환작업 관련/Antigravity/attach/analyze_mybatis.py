...
XML_ROOT = r"d:\panocean-v2-xml\src\main\resources\mappers\som"
JAVA_ROOTS = [
    r"d:\panocean-v2-xml\src\main\java\com\pan\som\dao",
    r"d:\panocean-v2-xml\src\main\java\com\pan\som\function",
    r"d:\panocean-v2-xml\src\main\java\com\pan\som\service",
]
OUTPUT_FILE = r"d:\mybatis_parameterType.md"
# ---------------------------------------------
...
UXBDAO_CALL_PATTERN = re.compile(
    r'uxbDAO\.(insert|update)\s*\(\s*["\']([^"\']+)["\']\s*,\s*([\w\.]+)',
    re.MULTILINE
)
...
def generate_md(xml_data, java_results):
    ...
    for xp in sorted(xml_data.keys(), key=lambda p: os.path.basename(p).lower()):
        ...
        for qid, func_str, fqcn_str, note in rows:
            lines += [
                '    <tr>\n',
                f'      <td style="padding:12px 15px;border:1px solid #dddddd;color:#000000;font-weight:bold;word-break:break-all;">{qid}</td>\n',
                f'      <td style="padding:12px 15px;border:1px solid #dddddd;color:#333333;word-break:break-all;font-size:12px;">{func_str}</td>\n',
                f'      <td style="padding:12px 15px;border:1px solid #dddddd;color:#0055aa;font-weight:bold;word-break:break-all;">{fqcn_str}</td>\n',
                f'      <td style="padding:12px 15px;border:1px solid #dddddd;color:#555555;font-size:12px;">{note}</td>\n',
                '    </tr>\n'
            ]
        lines.append('  </tbody>\n</table>\n\n')
